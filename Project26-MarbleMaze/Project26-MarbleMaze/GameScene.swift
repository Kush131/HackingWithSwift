...
//
//  Created by Kush, Ryan on 2/11/19.
                ...
                for (row, line) in lines.reversed().enumerated() {
                    for (column, letter) in line.enumerated() {
                        ...
                        if letter == "x" {
                            ...
                            node.physicsBody = .init(circleOfRadius: node.size.width / 2)
                            node.physicsBody?.isDynamic = false
                            node.physicsBody?.collisionBitMask  = CollisionTypes.vortex.rawValue
